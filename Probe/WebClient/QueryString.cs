    ...
    public class QueryString : NameValueCollection
    {
        ...
        public QueryString Add(string name, string value, bool isUnique)
        {
            ...
            if (string.IsNullOrEmpty(existingValue))
                base.Add(name, Uri.EscapeDataString(value));
            else if (isUnique)
                base[name] = Uri.EscapeDataString(value);
            else
                base[name] += "," + Uri.EscapeDataString(value);
            return this;
        }
        ...
        public new string this[string name]
        {
            get
            {
                return Uri.UnescapeDataString(base[name]);
            }
        }
        ...
        public new string this[int index]
        {
            get
            {
                return Uri.EscapeDataString(base[index]);
            }
        }
            ...
            for (var i = 0; i < base.Keys.Count; i++)
            {
                if (!string.IsNullOrEmpty(base.Keys[i]))
                {
                    foreach (string val in base[base.Keys[i]].Split(','))
                        builder.Append((builder.Length == 0) ? "?" : "&").Append(Uri.EscapeDataString(base.Keys[i])).Append("=").Append(val);
                }
            }
