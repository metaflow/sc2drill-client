...
{
    public class CrossVersionSettingProvider : SettingsProvider
    {
        ...
        readonly string APPNODE = System.Reflection.Assembly.GetExecutingAssembly().GetName().Name + ".Properties.Settings";
        ...
        public string SettingsPath
        {
            get
            {
                var currentDirecotry = new FileInfo(System.Reflection.Assembly.GetEntryAssembly().Location).DirectoryName;

                return Directory.GetParent(currentDirecotry).ToString();
            }
        }
        ...
        public override void SetPropertyValues(SettingsContext sContext, SettingsPropertyValueCollection settingsColl)
        {
            ...
            try
            {
                XMLConfig.Save(Path.Combine(SettingsPath, SettingsFileName));
            }
            catch (Exception ex)
            {
                //todo: Create an informational message for the user if we cannot save the settings.
        ...
        private XmlDocument XMLConfig
        {
            get
            {
                ...
                if (xmlDoc == null)
                {
                    ...
                    try
                    {
                        xmlDoc.Load(Path.Combine(SettingsPath, SettingsFileName));
                    }
